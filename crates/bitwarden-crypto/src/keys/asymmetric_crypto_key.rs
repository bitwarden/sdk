...
impl AsymmetricCryptoKey {
    ...
    pub fn from_pem(pem: SensitiveString) -> Result<Self> {
    ...
    pub fn from_der(der: SensitiveVec) -> Result<Self> {
        ...
        Ok(Self {
            key: Box::pin(
                RsaPrivateKey::from_pkcs8_der(der.expose()).map_err(|_| CryptoError::InvalidKey)?,
            ),
        })
    }
...
mod tests {
    ...
    use crate::{
        ...
        KeyDecryptable, SensitiveString,
    };

    #[test]
    fn test_asymmetric_crypto_key() {
        let pem_key_str = SensitiveString::test(
            "-----BEGIN PRIVATE KEY-----
...
-----END PRIVATE KEY-----",
        ...
        let der_key = AsymmetricCryptoKey::from_der(der_key_vec.clone()).unwrap();
    ...
    fn test_encrypt_public_decrypt_private() {
        let private_key = SensitiveString::test(concat!(
