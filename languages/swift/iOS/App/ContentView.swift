...
struct ContentView: View {
    ...
    init() {
        ...
        let http = URLSession(configuration: URLSessionConfiguration.default, delegate: ignoreHttpsDelegate, delegateQueue: nil)
        ...
        Task {
            ...
            let (preloginDataJson, _) = try await http.data(for: request(method: "POST",url: IDENTITY_URL + "accounts/prelogin", fn: {r in
                r.setValue("application/json", forHTTPHeaderField: "Content-Type")
                r.httpBody = try JSONEncoder().encode(PreloginRequest(email: EMAIL))
            }))
            let preloginData = try JSONDecoder().decode(PreloginResponse.self, from: preloginDataJson)
            
            let kdf: Kdf
            if preloginData.kdf == 0 {
                kdf = Kdf.pbkdf2(iterations: preloginData.kdfIterations)
            } else {
                kdf = Kdf.argon2id(iterations: preloginData.kdfIterations, memory: preloginData.kdfMemory!, parallelism: preloginData.kdfParallelism!)
            }
            
            let passwordHash = try await client.kdf().hashPassword(email: EMAIL, password: PASSWORD, kdfParams: kdf);
            ...
            let (loginDataJson, _) = try await http.data(for: request(method: "POST", url: IDENTITY_URL + "connect/token", fn: {r in
                r.setValue(EMAIL.data(using: .utf8)?.base64EncodedString(), forHTTPHeaderField: "Auth-Email")
                ...
                comp.queryItems = [
                    ...
                    URLQueryItem(name: "deviceIdentifier", value: "0745d426-8dab-484a-9816-4959721d77c7"),
            ...
            let (syncDataJson, _) = try await http.data(for: request(method: "GET", url: API_URL + "sync?excludeDomains=true", fn: {r in
                r.setValue("Bearer " + loginData.access_token, forHTTPHeaderField: "Authorization")
            }))
            ...
            try await client.crypto().initializeCrypto(req: InitCryptoRequest(
                kdfParams: kdf,
                email: EMAIL,
                password: PASSWORD,
                userKey: loginData.Key,
                privateKey: loginData.PrivateKey,
                organizationKeys: Dictionary.init(uniqueKeysWithValues: syncData.profile.organizations.map {($0.id, $0.key) })
            ));
            ...
            let decryptedFolders = try await client.vault().folders().decryptList(folders: syncData.folders.map { Folder(id: $0.id, name: $0.name, revisionDate: dateFormatter.date(from: $0.revisionDate)!) })
...
func request(method: String, url: String, fn: (inout URLRequest) throws -> ()) -> URLRequest {
